# BUILD: TENNIS EDGE PRO V14 - MAIN.PY
import os
import html
import textwrap
import sqlite3
...
def aplicar_estilo_premium():
    st.markdown(
        ...
        /* V14: no eliminamos el header de Streamlit.
           El sidebar podrá cerrarse sin perder su control nativo de reapertura. */
        ...
        /* =====================================================
           ...
           ===================================================== */
        /* V14: navegación superior usa widgets nativos sin CSS por key. */
...
# =========================================================
# V14 · NAVEGACIÓN NATIVA Y SIEMPRE VISIBLE
# =========================================================
NAV_OPTIONS = [
    "⌂  Dashboard",
    "▣  Próximos partidos",
...
if "tep_page" not in st.session_state:
    st.session_state["tep_page"] = NAV_OPTIONS[0]

if "tep_top_nav" not in st.session_state:
    st.session_state["tep_top_nav"] = st.session_state["tep_page"]

if "tep_side_nav" not in st.session_state:
    st.session_state["tep_side_nav"] = st.session_state["tep_page"]


def _sync_from_top():
    st.session_state["tep_page"] = st.session_state["tep_top_nav"]
    st.session_state["tep_side_nav"] = st.session_state["tep_top_nav"]


def _sync_from_side():
    st.session_state["tep_page"] = st.session_state["tep_side_nav"]
    st.session_state["tep_top_nav"] = st.session_state["tep_side_nav"]


# PRUEBA/BUILD VISIBLE:
# Este bloque usa widgets NATIVOS de Streamlit, sin HTML y sin CSS específico.
# Si main.py está siendo ejecutado, tiene que aparecer.
st.caption("📱 Tennis Edge Pro · V14 navegación activa")

st.selectbox(
    "☰ MENÚ · Ir a",
    NAV_OPTIONS,
    key="tep_top_nav",
    on_change=_sync_from_top,
)

pagina_actual = st.session_state["tep_page"]
...
with st.sidebar:
    st.markdown(
        ...
        <div class="tep-brand">
          ...
          <div>
            <div class="tep-brand-title">Tennis Edge <span>Pro</span></div>
            <div class="tep-brand-sub">Prediction & Value Engine</div>
          </div>
        </div>
        """,
        unsafe_allow_html=True
